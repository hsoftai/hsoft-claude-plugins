...
## Claude Cowork (sealed-box disk channel)

In Cowork, commands run in an isolated Linux VM with no vault CLI and no network to
the host, while the hooks run on the host. secrets-guard resolves references **on
the host** and delivers the values to the VM over the **only** host↔VM channel — the
shared `outputs` disk — using an asymmetric **sealed box**, so the value lives
**only in the child process's memory** and never touches the VM's disk, shell, argv,
env, or the agent transcript. It is auto-detected (`CLAUDE_CODE_IS_COWORK`); plain
Claude Code is unaffected. See [docs/cowork.md](docs/cowork.md).
